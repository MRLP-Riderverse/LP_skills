...
- User wants video/audio/3D generation via AnimateDiff, Hunyuan, Wan, or AudioCraft

### Audio workflows

Use ComfyUI as the umbrella when the job is broader than just raw audio generation:
- MusicGen / AudioGen / AudioCraft workflows
- Audio conditioning, synthesis, or post-processing inside a larger workflow
- Mixing audio generation with image/video steps in the same pipeline

For standalone text-to-music or text-to-sound tasks, treat AudioCraft as a ComfyUI-compatible sub-workflow rather than a separate skill.
