...
metadata:
  hermes:
    tags: [X, Twitter, API, post, search, DM, v2, x-cli]
    related_skills: []
    absorbed_skills: [xitter]
    note: "xitter (x-cli terminal client) consolidated into xurl. For x-cli usage, set X_API_KEY/X_API_SECRET/X_BEARER_TOKEN/X_ACCESS_TOKEN/X_ACCESS_TOKEN_SECRET env vars and use `x` CLI commands for posting, timelines, likes, retweets, bookmarks."
