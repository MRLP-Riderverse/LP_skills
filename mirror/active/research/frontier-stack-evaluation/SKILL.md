...
- For delta briefs, compare against the last 3 delivered briefings before drafting; repeated items are stale unless there is a concrete new release/post/commit/status change
- For this workflow, the canonical skill name is `frontier-stack-evaluation`; aliases in skill frontmatter may not resolve in `skill_view`
...
7. **If a source blocks default view**, try plain/simplified variant before giving up
8. **Don’t overstate "latest"** if only older but still relevant posts are available
9. **Avoid using broad hype language** - keep output builder-facing and implementation-oriented
10. **RSS feeds can be partially useful even when spotty** — Solana, Ollama, and LangChain may alternate between full items and thin output. Treat an empty result as a signal to fall back, not as proof of silence.
11. **Google News RSS often returns zero results** for niche/technical queries (CRDT, local-first, specific protocols). This means the query format doesn't match indexed content, not that there's nothing happening. Fall back to canonical project blogs/repos directly.
12. **Solana RSS may be live enough to use directly** — when it returns full items with titles/descriptions/dates, prefer it over fallback scraping.
...
14. **LangChain blog posts are JS-rendered Webflow** — individual blog post URLs (e.g. `/blog/the-missing-link-between-agents-and-applications`) return 404 on curl/HTTP fetch because the site is a single-page Webflow app. RSS titles and descriptions are the reliable extraction path. If you need article body content, you need a browser/rendering engine — curl alone cannot extract it.
15. **OpenJarvis freshness signal is the canonical repo, not the desktop release feed** — the authoritative repository is `open-jarvis/OpenJarvis`; commit activity can be more current than packaged releases.
16. **x402 freshness signal is the writing page + repo commits** — the `/writing` page surfaces batch-settlement and ecosystem updates even when releases are sparse.
17. **Daily briefing freshness should be anchored to the last delivered brief** — read the previous brief’s content from the most recent matching session before drafting deltas.
