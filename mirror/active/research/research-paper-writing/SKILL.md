...
- **Searching for relevant papers** and gathering source metadata before drafting
- **Maintaining a persistent literature wiki or note graph** for a project

### Literature discovery and source intake

Use arXiv / Semantic Scholar when you need to find or verify papers by keyword, author, category, or ID. Use a persistent wiki-style knowledge base when you want the findings to compound over time instead of being rediscovered from scratch.

- **arXiv search**: find candidate papers, abstracts, and PDF links
- **Reading and synthesis**: pull the abstract or full paper, then distill claims and methods into the draft
- **LLM wiki maintenance**: file the source into an interlinked markdown knowledge base, cross-reference related concepts, and keep contradictions visible

These steps live here because paper writing starts before prose: it starts with finding, reading, and organizing the literature.
