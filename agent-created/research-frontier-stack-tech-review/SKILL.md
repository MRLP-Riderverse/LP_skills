...
- **`.dev` TLD security blocks:** Domains using `.dev` TLD (e.g. `helius.dev`) trigger a "lookalike TLD" security warning in `terminal()`. Use `curl` directly (not `&& echo` chaining) to fetch via browser if the terminal command is blocked.
- **Post-briefing message truncation (CRITICAL):** When running as a cron job, the delivery system captures the LAST assistant message — not the longest or most important one. If the agent produces the briefing, then appends a follow-up ("Briefing delivered. All five sources fetched..."), THAT follow-up becomes the delivered output, replacing the briefing. The briefing itself is lost. **Prevention:** the cron prompt must include an explicit rule: "Your briefing IS your final response. Do NOT add any follow-up, summary, or closing message after the briefing." Test this after any prompt changes by checking the cron output file size against previous days.
- **Multilingual hallucination (CRITICAL):** When source material contains non-English text (e.g. ForkLog is Dutch), the model can duplicate entire sections in that language. This produces garbled output on Telegram and wastes the delivery budget. **Prevention:** the cron prompt must include: "Write the ENTIRE briefing in English ONLY. Never duplicate sections in other languages." If source material is in another language, translate to English before including.
- **Telegram table stripping:** Markdown tables (pipe-delimited) are stripped or garbled by Telegram's renderer. Use bullet lists instead. This applies to any briefing delivered to Telegram.
...
- See `references/2026-06-10-session-notes.md` for cron delivery truncation bug (post-briefing message replaces briefing), Dutch/multilingual section duplication in music brief, delta-only briefing mode design, and LP_skills sync.
